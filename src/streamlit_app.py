...
def get_user_inputs():
    """Get and validate user inputs."""
     # --- Inputs ---
    lat = st.number_input("Latitude", value=55.9533, min_value=-90.0, max_value=90.0, 
                         help="Latitude coordinate (-90 to 90). Default: Edinburgh, Scotland")
    lon = st.number_input("Longitude", value=-3.1883, min_value=-180.0, max_value=180.0,
    ...
    if date_range > 30:
        st.warning(f"Selected date range is {date_range} days. Large date ranges may result in slower performance.")
    # Validate date range
...
def fetch_weather_data(inputs):
    ...
    try:
        ...
        st.error("Request timed out. Please try again or check your internet connection.")
        return None
